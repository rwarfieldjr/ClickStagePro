...
app.use((req, res, next) => {
  ...
  if (req.path === '/api/stripe-webhook' || req.path === '/api/webhooks/stripe' || req.path === '/api/billing/webhook') {
    return next();
  }
...
app.post("/api/uploads/presign", async (req, res) => {
  try {
    ...
    if (!filename || !mime) {
      return res.status(400).json({ ok:false, error:"Missing filename or mime" });
    }
    // Basic size/type guard
    const MAX = 10 * 1024 * 1024; // 10MB
    if (size && size > MAX) return res.status(413).json({ ok:false, error:"File too large" });
    if (!/^image\/(png|jpe?g|webp|gif)$/i.test(mime)) {
      return res.status(415).json({ ok:false, error:"Unsupported file type" });
    }
    if (!process.env.CF_ACCOUNT_ID || !process.env.R2_ACCESS_KEY_ID || !process.env.R2_SECRET_ACCESS_KEY || !process.env.R2_BUCKET) {
    ...
    return res.status(500).json({ ok:false, error:"Server error presigning" });
  }
});
...
["CF_ACCOUNT_ID","R2_ACCESS_KEY_ID","R2_SECRET_ACCESS_KEY","R2_BUCKET"].forEach(k=>{
...
app.post("/api/submit", async (req, res) => {
  try {
    ...
    if (!payload?.email || !Array.isArray(payload?.file_urls) || payload.file_urls.length === 0) {
      return res.status(400).json({ ok: false, error: "Missing email or file_urls" });
    }
...
app.get("/dev/health", (_req, res) => {
  ...
  const ok = info.present.R2_BUCKET && info.present.R2_ACCESS_KEY_ID && info.present.R2_SECRET_ACCESS_KEY;
...
async function uploadProxyHandler(req: Request, res: Response) {
  try {
    ...
    console.log(`[upload ${(req as any).rid}] filename=${filename} mime=${mime} size=${size}`);
    if (!filename || !mime || !fileData) {
      return res.status(400).json({ ok: false, error: "Missing filename, mime, or fileData" });
    }
...
app.post("/api/r2/upload-url", async (req, res) => {
  ...
  try {
    ...
    console.log(`R2 upload URL generated for user ${userId}, file: ${filename}, key: ${key} (${duration}ms)`);
...
app.post("/api/r2/download-url", async (req, res) => {
  ...
  try {
    ...
    const cmd = new GetObjectCommand({
      Bucket: R2_BUCKET,
      Key: key,
      ResponseContentDisposition: asFilename ? `attachment; filename="${asFilename}"` : undefined,
    });
...
app.post("/api/ai/property-description", async (req, res) => {
  try {
    ...
    if (!dataUrlMatch) {
      return res.status(400).json({ error: "Invalid image format. Must be PNG, JPEG, WebP, or GIF" });
    }
...
app.post("/api/ai/chat", async (req, res) => {
  try {
    ...
    const response = await aiService.chatWithSupport(message.slice(0, 2000), conversationHistory);
...
app.post('/api/create-checkout-session', async (req, res) => {
  try {
    ...
    if (isTestKey && !isDevEnvironment) {
      console.error('[CHECKOUT] Refusing to create live checkout with TEST key');
      return res.status(400).json({ 
        ok: false, 
        message: 'Server misconfigured: test key detected. Please contact support.'
      });
    }
    
    const priceId = PRICE_MAP[bundle];
    if (!priceId || !priceId.startsWith('price_')) {
      console.error('[CHECKOUT] Missing/invalid priceId', { bundle, priceId, PRICE_MAP });
      return res.status(400).json({ 
        ok: false, 
    ...
    console.log('[CHECKOUT] Creating session:', { bundle, priceId: priceId.substring(0, 20) + '...', mode: 'live' });
...
app.get('/api/debug/stripe', (req, res) => {
  ...
  const prices = Object.fromEntries(
    Object.entries(PRICE_MAP).map(([k, v]) => [k, Boolean(v && v.startsWith('price_'))])
  );
  res.json({
    ok: true,
  ...
  import("./storage").then(async ({ storage }) => {
    ...
    try {
      ...
      if (!existingUser) {
        ...
        console.log(`[DEV AUTH] Dev user already exists in database: ${devUserId}`);
      }
    } catch (error) {
    ...
    const scheduleNightlyCleanup = () => {
      ...
      setTimeout(async () => {
        try {
          ...
          console.log(`[nightly cleanup] Expired ${expiredCount} credit balances`);
        } catch (err) {
